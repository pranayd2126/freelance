      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-stone-900 text-white rounded-2xl p-6 md:p-8 shadow-md border border-stone-850">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-stone-300 mt-1 text-sm md:text-base font-medium">Manage your active projects, submit bids, and track your freelance earnings.</p>
          </div>
        </div>
        <div className="flex items-center">
          <Link to="/projects">
            <Button variant="primary" icon={HiArrowRight} className="bg-orange-600 hover:bg-orange-700">
              Browse Projects
            </Button>
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-stone-850">Active Projects</h2>
          </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-stone-500 mt-1 font-medium">
                          Posted by <span className="font-bold text-stone-700">{project.client?.companyName || project.client?.user?.fullName || 'Client'}</span>
                        </p>
                        <div className="flex gap-2 items-center mt-2 flex-wrap">
                          <span className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">Advance Payout:</span>
                          {projectPayment ? (
                            <Badge variant={projectPayment.status === 'released' ? 'success' : 'warning'} className="text-[10px] py-0.5 font-bold">
                              {projectPayment.status === 'released' ? 'Released to Bank' : 'Held in Escrow'}
                            </Badge>
                          ) : (
                            <Badge variant="default" className="text-[10px] py-0.5 font-bold">Unpaid</Badge>
                          )}
                        </div>
                      </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-stone-850">Recent Bids</h2>
            <Link to="/dashboard/my-bids" className="text-sm font-bold text-orange-650 hover:text-orange-800">
              View All
            </Link>
          </div>
          ...
          {myBids && myBids.length > 0 ? (
            <div className="space-y-3">
              {myBids.slice(0, 5).map((bid) => {
                ...
                return (
                  <Card key={bid._id} className="p-4 border border-stone-200/80">
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <h4 className="text-sm font-bold text-stone-800 line-clamp-1 hover:text-orange-655">
                          <Link to={`/projects/${bid.project?._id || bid.project}`}>{bid.project?.title || 'Project Details'}</Link>
                        </h4>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-stone-500 font-semibold">Bid: {formatCurrency(bid.amount)}</span>
                          <span className="text-xs text-stone-300">•</span>
                          <span className="text-xs text-stone-500 font-semibold">{new Date(bid.createdAt).toLocaleDateString()}</span>
                        </div>
                      </div>
