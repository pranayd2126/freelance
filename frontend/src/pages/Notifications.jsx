      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
            <HiBell className="text-primary-600" />
            Notifications
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            You have <span className="font-bold text-primary-700">{unreadCount}</span> unread notification{unreadCount !== 1 ? 's' : ''}.
          </p>
        </div>
        {unreadCount > 0 && (
          <Button 
          ...
          {sortedNotifications.map((n) => (
            <Card
              ...
              className={`p-4 border transition-all cursor-pointer relative overflow-hidden ${
                !n.isRead 
                  ? 'border-primary-100 bg-primary-50/30 hover:bg-primary-50/50 shadow-sm' 
            ...
            >
              ...
              <div className="flex gap-3">
                <div className={`p-2 rounded-lg h-fit ${!n.isRead ? 'bg-primary-100 text-primary-600' : 'bg-slate-100 text-slate-400'}`}>
