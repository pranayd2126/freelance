...
const ProjectDetails = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link 
        to="/projects" 
        className="inline-flex items-center text-sm font-bold text-stone-600 hover:text-stone-900 mb-6 transition-colors"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-sm">
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-stone-900 mb-4 tracking-tight leading-tight">
              {project.title}
            </h1>

            <div className="prose max-w-none text-stone-650 mb-6 whitespace-pre-line leading-relaxed font-medium">
              <h3 className="text-lg font-bold text-stone-850 mb-2">Project Description</h3>
              {project.description}
            </div>

            <div>
              <h3 className="text-sm font-bold text-stone-850 mb-2">Skills Required</h3>
              <div className="flex flex-wrap gap-2">
                {projectSkills.map((skill, index) => (
                  <Badge key={index} variant="default" className="bg-stone-100 text-stone-700 font-semibold">
                    {skill}
                  </Badge>
                ))}
              </div>
          ...
          {isOwner && (
            <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-sm">
              <h2 className="text-xl font-bold text-stone-900 mb-6 pb-3 border-b border-stone-100">
              ...
              {isBidsLoading ? (
                ...
                <div className="text-center py-12 text-stone-500">
                  <HiDocumentText className="mx-auto h-12 w-12 text-stone-350 mb-3" />
                  <p className="font-bold text-stone-800">No bids submitted yet</p>
                  <p className="text-xs font-semibold">Bids from students will appear here once submitted.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {projectBids.map((bid) => (
                    <Card key={bid._id} className="border-stone-150 bg-stone-50/50">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-xs text-stone-500 mb-3 font-semibold">
                            ...
                            <span className="flex items-center gap-1">
                              <HiClock className="h-4 w-4 text-stone-405" />
                          ...
                          <div className="text-sm text-stone-650 bg-white p-3 rounded-lg border border-stone-150 mb-3 whitespace-pre-line leading-relaxed font-medium">
                            <span className="font-bold text-xs text-stone-500 block mb-1">COVER LETTER:</span>
                            {bid.coverLetter}
                          </div>

                          {bid.portfolioLinks?.length > 0 && (
                            <div className="mb-3">
                              <span className="font-bold text-xs text-stone-500 block mb-1">PORTFOLIO LINKS:</span>
                              <div className="flex flex-wrap gap-2">
                                {bid.portfolioLinks.map((link, idx) => (
                                  <a 
                                    ...
                                    className="inline-flex items-center gap-1 text-xs text-orange-600 hover:text-orange-850 hover:underline bg-orange-50/50 px-2 py-1 rounded font-bold"
                                  >
                        ...
                        {project.status === 'open' && bid.status === 'pending' && (
                          <div className="flex sm:flex-col gap-2 min-w-[120px]">
                            ...
                            <Button 
                              size="sm" 
                              variant="outline" 
                              className="text-rose-650 border-rose-200 hover:bg-rose-50"
          ...
          {isStudent && (
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              {studentBid ? (
                <div>
                  ...
                  <div className="bg-orange-50/40 p-4 rounded-xl border border-orange-100 space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-stone-700">Bid Status</span>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="text-xs text-stone-500 block">YOUR BID AMOUNT</span>
                        <span className="text-lg font-bold text-stone-800">{formatCurrency(studentBid.amount)}</span>
                      </div>
                      <div>
                        <span className="text-xs text-stone-500 block">ESTIMATED TIME</span>
                        <span className="text-lg font-bold text-stone-800">{studentBid.estimatedTime}</span>
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-stone-500 block mb-1">YOUR COVER LETTER</span>
                      <p className="text-sm text-stone-650 bg-white p-3 rounded-lg border border-stone-150 whitespace-pre-line leading-relaxed font-medium">
                    ...
                    {studentBid.portfolioLinks?.length > 0 && (
                      <div>
                        <span className="text-xs text-stone-500 block mb-1">PORTFOLIO LINKS</span>
                        <div className="flex flex-wrap gap-2">
                          {studentBid.portfolioLinks.map((link, idx) => (
                            <a 
                              ...
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 text-xs text-orange-650 hover:text-orange-850 hover:underline bg-white px-3 py-1.5 rounded-lg border border-stone-205 font-bold"
                            >
                              <HiExternalLink className="h-3 w-3" />
                              Portfolio {idx + 1}
                            </a>
                ...
                <div className="text-center py-6 text-stone-500 font-semibold">
                  <p className="font-bold text-stone-800">Bidding is Closed</p>
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="text-xs text-stone-400 block font-bold">BUDGET RANGE</span>
              ...
              <div className="flex items-start gap-3">
                <HiCalendar className="h-5 w-5 text-orange-500 mt-0.5" />
                <div>
                  <span className="text-xs text-stone-400 block font-bold">DEADLINE</span>
                  <span className="text-sm font-bold text-stone-850">
              ...
              <div className="flex items-start gap-3">
                <HiBriefcase className="h-5 w-5 text-stone-500 mt-0.5" />
                <div>
                  <span className="text-xs text-stone-400 block font-bold">BIDS RECEIVED</span>
                  <span className="text-sm font-bold text-stone-850">
              ...
              <div className="flex items-start gap-3">
                <HiUser className="h-5 w-5 text-amber-500 mt-0.5" />
                <div>
                  <span className="text-xs text-stone-400 block font-bold">POSTED BY</span>
                  <span className="text-sm font-bold text-stone-850">
