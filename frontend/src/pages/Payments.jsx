    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
          <HiCreditCard className="text-primary-600 h-8 w-8" /> Payment Dashboard
        </h1>
                  ...
                  <Card key={project._id} className="space-y-4">
                    <div className="flex justify-between items-start border-b border-slate-100 pb-4">
                      <div>
                        ...
                        <p className="text-xs font-medium text-primary-600">Assigned Student: {project.assignedTo?.name || 'Unassigned'}</p>
                      </div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {payments.map((payment) => (
                  <tr key={payment._id} className="hover:bg-slate-50/80 transition-colors">
                     ...
                     <td className="p-4 font-bold text-primary-600">{formatCurrency(payment.amount)}</td>
          ...
          <Card className="max-w-md w-full shadow-2xl relative">
            <h3 className="text-lg font-bold text-slate-800 mb-2">Confirm Payment</h3>
             <p className="text-xs text-slate-500 mb-6">
               You are about to deposit <span className="font-bold text-primary-600">{formatCurrency(checkoutMilestone.amount)}</span> into escrow for milestone <span className="font-bold">"{checkoutMilestone.title}"</span>.
             </p>
