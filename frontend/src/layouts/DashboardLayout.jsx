...
const DashboardLayout = () => {
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col justify-between bg-white border-r border-stone-200">
      <div>
        {/* Sidebar Header */}
        <div className="h-16 flex items-center px-6 border-b border-stone-200">
          <Link to="/" className="text-xl font-extrabold bg-gradient-to-r from-orange-600 to-amber-500 bg-clip-text text-transparent">
            StudentBid
          </Link>
        </div>
        ...
        <div className="p-4 border-b border-stone-100 flex items-center gap-3">
          ...
          <div className="overflow-hidden leading-tight">
            <h4 className="font-bold text-stone-850 truncate text-sm">{user?.fullName}</h4>
            <p className="text-xs text-stone-500 capitalize font-medium">{user?.role}</p>
          </div>
        </div>
          ...
          {menuLinks.map((link) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-semibold transition-colors ${
                    isActive
      ...
      <div className="p-4 border-t border-stone-100">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-semibold text-red-650 hover:bg-red-50 transition-colors disabled:opacity-50 text-left cursor-pointer"
        >
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <div className="flex-1 md:pl-64 flex flex-col min-h-screen">
        {/* Top bar */}
        <header className="h-16 bg-white border-b border-stone-200 px-6 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setMobileOpen(!mobileOpen)}
              className="p-2 -ml-2 rounded-lg text-stone-600 hover:bg-stone-100 focus:outline-none md:hidden"
            >
              {mobileOpen ? <HiX className="h-6 w-6" /> : <HiMenu className="h-6 w-6" />}
            </button>
            <span className="font-bold text-stone-850 text-lg hidden md:block">
              {getPageTitle()}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <Link
              ...
              className="relative text-stone-600 hover:text-orange-600 transition-colors p-2 rounded-full hover:bg-stone-50"
            >
            ...
            <hr className="h-6 w-px bg-stone-200" />
            <div className="flex items-center gap-2">
              <Avatar name={user?.fullName} src={user?.avatar?.url} size="sm" />
              <span className="text-sm font-semibold text-stone-700 hidden lg:block">
                {user?.fullName}
              </span>
            </div>
          </div>
